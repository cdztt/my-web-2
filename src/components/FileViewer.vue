...
const currentFileIndex = computed(() => files.value?.findIndex(fileName => fileName === route.params.fileName))
...
const handleClick = (which) => {
    if (which === 'prev') {
        router.replace(route.path.split('/').slice(0, -1).concat(prevFile.value).join('/'))
    }
    else if (which === 'next') {
        router.replace(route.path.split('/').slice(0, -1).concat(nextFile.value).join('/'))
    }
}
    ...
    <div class="fileviewer">
        <div className='fileviewer-title'>
            <div className='fileviewer-title-arrow'>
                <span :class="['fileviewer-title-arrow-entity', { invisible: !prevFile }]"
                ...
                <span :class="['fileviewer-title-arrow-popup', { visible: isPopup.prev }]">
                    上一篇：{{ prevFile }}
                </span>
            </div>
            ...
            <div className='fileviewer-title-arrow'>
                <span :class="['fileviewer-title-arrow-entity', { invisible: !nextFile }]"
                ...
                <span :class="['fileviewer-title-arrow-popup', { visible: isPopup.next }]">
                    下一篇：{{ nextFile }}
                </span>
            </div>
        </div>
        ...
        <div className='fileviewer-time'>
            {{ file?.createdAt ? new Date(file.createdAt).toISOString().slice(0, 10) : '' }}
        </div>
